...
@onready var gunRay = $Head/Camera/RayCast3d as RayCast3D
@onready var Cam = $Head/Camera as Camera3D
...
var t = "Agent%d: Total Reward: %d, State: %d \n    Enemy_pos_y %.2f, Enemy_pos_y %.2f, Player_rot_x %.2f, Player_rot_y %.2f"
var alpha = 1
...
func _physics_process(delta):
	alpha -= 0.01
	$Reward.add_theme_color_override("font_color", Color(0, 1, 0, alpha))
	if not is_on_floor():
...
func play_action(action):
	...
	if action == "shoot":
		...
		$Head.rotation.x -= 100.0 / mouseSensibility
...
#	bulletInst.look_at((gunRay.get_collision_point()+gunRay.get_collision_normal()),Vector3.BACK)
	...
	if collided_object and "Reward" in collided_object.name:
#			collided_object.emit_signal("bullet_collision")
		reward = int(collided_object.name.replace("Reward", ''))
		total_reward += reward
...
func save_state(reward, done):
	var data_to_send = {
		"reward" : reward,
		"player_rotation_x" : $Head.rotation.x,
...
func update_info():
	$Info.text = t % [agent_n, total_reward, GlobalVars.state_num, enemy.position.y, enemy.position.z, $Head.rotation.x, self.rotation.y]
