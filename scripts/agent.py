...
class Agent:
    ...
    def load_model(self, state):
        ...
        if os.path.exists(f"./info_models/agent{self.agent_n}/{model_name}"):
            try:
                state_tensor = tf.convert_to_tensor(state)
                state_tensor = tf.expand_dims(state_tensor, 0)
                self.model(state_tensor, training=False)
                self.model.load_weights(f"./info_models/agent{self.agent_n}/{model_name}")
    ...
    def load_info(self):
        # info_name = 'info.txt'
        info_name = 'pretrained_info.txt'
        if os.path.exists(f"./info_models/{info_name}"):
            with open(f"./info_models/{info_name}", "r") as f:
                lines = f.readlines()
